...
const permissionMapSchema = z.record(z.array(z.enum(RBAC_ACTIONS as unknown as [string, ...string[]])).default([]));
...
rootRoleAccessRouter.get('/orgs/:orgId/rbac/version', async (req, res, next) => {
  try {
    ...
    const isSuperAdmin = normalizedRoleKey === 'SUPER_ADMIN' || normalizedRoles.includes('SUPER_ADMIN');
