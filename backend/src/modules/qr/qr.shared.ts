...
export function toScanLinks(req: RequestLike, token: string, assetId?: string) {
  const frontendBase = publicFrontendBaseUrl(req);
  return {
    publicResolverUrl: `${frontendBase}/qr/${encodeURIComponent(token)}`,
    appScanUrl: `${frontendBase}/scan/${encodeURIComponent(token)}`,
    machineCardUrl: assetId ? `${frontendBase}/machine/${encodeURIComponent(assetId)}` : `${frontendBase}/machine`,
  };
}
...
export function toResolvedPayload(req: RequestLike, token: string, row: NonNullable<Awaited<ReturnType<typeof findQrResolutionRow>>>) {
  return {
    ...
    links: toScanLinks(req, token, row.asset_id),
  };
}
