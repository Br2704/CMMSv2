...
export class AddAdvancedAmcModule1700000000025 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    const amcColumns = [
      { name: 'contract_name', type: 'varchar', isNullable: true },
      { name: 'contract_type', type: 'varchar', isNullable: true },
      { name: 'visit_frequency', type: 'varchar', isNullable: true },
      { name: 'response_time_sla', type: 'int', isNullable: true },
      { name: 'resolution_time_sla', type: 'int', isNullable: true },
      { name: 'contract_value', type: 'decimal', precision: 12, scale: 2, isNullable: true },
    ];

    for (const col of amcColumns) {
      if (!(await queryRunner.hasColumn('amc_contracts', col.name))) {
        await queryRunner.addColumn('amc_contracts', new TableColumn(col));
      }
    }

    if (!(await queryRunner.hasTable('amc_contract_machines'))) {
    ...
    if (!(await queryRunner.hasTable('amc_visit_schedule'))) {
      await queryRunner.createTable(
        new Table({
    ...
    if (!(await queryRunner.hasTable('amc_service_reports'))) {
      await queryRunner.createTable(
        new Table({
    ...
    if (!(await queryRunner.hasTable('vendor_user_mappings'))) {
      await queryRunner.createTable(
        new Table({
