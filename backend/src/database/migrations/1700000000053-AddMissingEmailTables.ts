...
export class AddMissingEmailTables1700000000053 implements MigrationInterface {
  name = 'AddMissingEmailTables1700000000053';
      ...
      await queryRunner.createTable(
        new Table({
          ...
          columns: [
            ...
            { name: 'sent_at', type: DATETIME_COLUMN_TYPE, isNullable: true },
            { name: 'opened_at', type: DATETIME_COLUMN_TYPE, isNullable: true },
      ...
      await queryRunner.createTable(
        new Table({
          ...
          columns: [
            ...
            { name: 'next_retry_at', type: DATETIME_COLUMN_TYPE, isNullable: true },
            { name: 'processed_at', type: DATETIME_COLUMN_TYPE, isNullable: true },
            { name: 'template_name', type: 'varchar', isNullable: true },
            { name: 'template_data', type: 'text', isNullable: true },
            { name: 'wo_id', type: 'uuid', isNullable: true },
            { name: 'wo_number', type: 'varchar', isNullable: true },
            { name: 'event_type', type: 'varchar', isNullable: true },
            { name: 'created_at', type: DATETIME_COLUMN_TYPE, default: 'CURRENT_TIMESTAMP' },
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    for (const tableName of ['email_logs', 'mail_queue']) {
