import {
  AmcContractEntity,
  AmcContractMachineEntity,
  AmcVisitScheduleEntity,
  AmcServiceReportEntity,
  AssetEntity,
  AssetQrEntity,
  AssetDowntimeEventEntity,
  AssetReliabilityKpiEntity,
  EmailLogEntity,
  MailQueueEntity,
  EscalationHistoryEntity,
  SlaConfigEntity,
  NotificationSettingsEntity,
  PushSubscriptionEntity,
  AssetPerformanceLogEntity,
  AssetEnergyMeterConfigEntity,
  FeatureFlagEntity,
  AlertConfigEntity,
  AlertLogEntity,
  CalibrationTemplateEntity,
  CalibrationRecordEntity,
  CostCenterEntity,
  DepartmentEntity,
  MachineModuleEntity,
  MachineInstrumentEntity,
  EmailReportLogEntity,
  EmailReportScheduleEntity,
  EsgMetricEntity,
  EsgKpiMasterEntity,
  EsgTargetEntity,
  EsgEmissionFactorEntity,
  EsgAuthorizedUserEntity,
  EsgEnergyDataEntity,
  EsgWaterDataEntity,
  EsgEmissionDataEntity,
  EsgWasteDataEntity,
  EsgProductionDataEntity,
  EsgKpiResultEntity,
  EsgDailyEntryEntity,
  EsgMonthlySummaryEntity,
  EsgPlantTargetEntryEntity,
  EsgOrganizationTargetEntryEntity,
  EnergyMeterReadingEntity,
  EmissionsFactorEntity,
  GhgActivityDataEntity,
  InstrumentCalibrationScheduleEntity,
  InstrumentCalibrationTaskEntity,
  EsgReportEntity,
  GateEntryEntity,
  GateEntryTypeEntity,
  GateEntryTemplateEntity,
  GateTemplateFieldEntity,
  GateTemplateUserEntity,
  GateVehicleEntryEntity,
  GateMaterialEntryEntity,
  GateExitLogEntity,
  GateEntity,
  GhgTransportLogEntity,
  VisitorExperienceContentEntity,
  PlantLayoutEntity,
  PlantCoordinateEntity,
  PathwayEntity,
  GeoFenceEntity,
  VisitorSessionEntity,
  VisitorNavigationLogEntity,
  VisitorTrackingEntity,
  VisitorSafetyLogEntity,
  LogEntryEntity,
  LogEntryValueEntity,
  LogTemplateAssignmentEntity,
  LogTemplateEntity,
  LogTemplateFieldEntity,
  MaintenanceTeamEntity,
  NotificationEntity,
  SecurityEventEntity,
  OrgRbacMetaEntity,
  OrgRoleEntity,
  OrgRolePermissionEntity,
  OrganizationEntity,
  OrganizationFeatureEntity,
  PlantEntity,
  PmScheduleEntity,
  PmTemplateEntity,
  PmTemplateLinkEntity,
  ProfileEntity,
  RefreshTokenEntity,
  RoleEntity,
  RoleDashboardKpiEntity,
  RolePermissionEntity,
  SafetyIncidentEntity,
  SafetyMetricEntity,
  ShiftEntity,
  SpareItemEntity,
  StockRequestEntity,
  UserEntity,
  UserRoleEntity,
  VendorEntity,
  VendorNotificationSettingEntity,
  VendorUserMappingEntity,
  WorkOrderEntity,
  WorkOrderVerificationEntity,
  WorkOrderHandoverEntity,
  WorkOrderActivityLogEntity,
  WorkOrderMasterEntity,
  WorkOrderTeamMappingEntity,
  AuditLogEntity,
  BrandingMetaEntity,
  RbacMetaEntity,
  MaintenanceReportEntity,
  SystemConfigEntity,
  ReportFormatConfigEntity,
  BackupHistoryEntity,
  RestoreHistoryEntity,
  BackupAuditLogEntity,
  UserPlantMappingEntity,
  UserDepartmentMappingEntity,
  UserModuleMappingEntity,
  UserMachineMappingEntity,
  UserShiftMappingEntity,
  ChangeRequestEntity,
  RecordRevisionEntity,
  PendingExecutionEntity,
  ShiftHandoverEntity,
  FailureCodeEntity,
  RcaEntity,
  MachineFailureCodeMappingEntity,
  WarrantyAlertEntity,
} from './index';

export const ALL_ENTITIES = [
  OrganizationEntity,
  OrganizationFeatureEntity,
  OrgRoleEntity,
  OrgRolePermissionEntity,
  OrgRbacMetaEntity,
  BrandingMetaEntity,
  UserEntity,
  RoleEntity,
  RoleDashboardKpiEntity,
  UserRoleEntity,
  RefreshTokenEntity,
  PlantEntity,
  ProfileEntity,
  RolePermissionEntity,
  DepartmentEntity,
  MachineModuleEntity,
  MachineInstrumentEntity,
  CostCenterEntity,
  VendorEntity,
  ShiftEntity,
  AssetEntity,
  AssetQrEntity,
  AssetDowntimeEventEntity,
  AssetReliabilityKpiEntity,
  AssetPerformanceLogEntity,
  AssetEnergyMeterConfigEntity,
  FeatureFlagEntity,
  AlertConfigEntity,
  AlertLogEntity,
  WorkOrderEntity,
  WorkOrderVerificationEntity,
  WorkOrderHandoverEntity,
  WorkOrderActivityLogEntity,
  WorkOrderMasterEntity,
  WorkOrderTeamMappingEntity,
  PmScheduleEntity,
  PmTemplateEntity,
  PmTemplateLinkEntity,
  SpareItemEntity,
  StockRequestEntity,
  CalibrationTemplateEntity,
  InstrumentCalibrationScheduleEntity,
  InstrumentCalibrationTaskEntity,
  CalibrationRecordEntity,
  AmcContractEntity,
  AmcContractMachineEntity,
  AmcVisitScheduleEntity,
  AmcServiceReportEntity,
  GateEntity,
  GateEntryEntity,
  GateEntryTypeEntity,
  GateEntryTemplateEntity,
  GateTemplateFieldEntity,
  GateTemplateUserEntity,
  GateVehicleEntryEntity,
  GateMaterialEntryEntity,
  GateExitLogEntity,
  GhgTransportLogEntity,
  VisitorExperienceContentEntity,
  PlantLayoutEntity,
  PlantCoordinateEntity,
  PathwayEntity,
  GeoFenceEntity,
  VisitorSessionEntity,
  VisitorNavigationLogEntity,
  VisitorTrackingEntity,
  VisitorSafetyLogEntity,
  NotificationEntity,
  SecurityEventEntity,
  MaintenanceTeamEntity,
  LogTemplateEntity,
  LogTemplateFieldEntity,
  LogTemplateAssignmentEntity,
  LogEntryEntity,
  LogEntryValueEntity,
  EsgMetricEntity,
  EsgKpiMasterEntity,
  EsgTargetEntity,
  EsgEmissionFactorEntity,
  EsgAuthorizedUserEntity,
  EsgEnergyDataEntity,
  EsgWaterDataEntity,
  EsgEmissionDataEntity,
  EsgWasteDataEntity,
  EsgProductionDataEntity,
  EsgKpiResultEntity,
  EsgDailyEntryEntity,
  EsgMonthlySummaryEntity,
  EsgPlantTargetEntryEntity,
  EsgOrganizationTargetEntryEntity,
  EnergyMeterReadingEntity,
  EmissionsFactorEntity,
  GhgActivityDataEntity,
  EsgReportEntity,
  SafetyIncidentEntity,
  SafetyMetricEntity,
  EmailReportScheduleEntity,
  EmailReportLogEntity,
  VendorNotificationSettingEntity,
  VendorUserMappingEntity,
  AuditLogEntity,
  RbacMetaEntity,
  PushSubscriptionEntity,
  EmailLogEntity,
  MailQueueEntity,
  EscalationHistoryEntity,
  SlaConfigEntity,
  NotificationSettingsEntity,
  MaintenanceReportEntity,
  SystemConfigEntity,
  ReportFormatConfigEntity,
  BackupHistoryEntity,
  RestoreHistoryEntity,
  BackupAuditLogEntity,
  UserPlantMappingEntity,
  UserDepartmentMappingEntity,
  UserModuleMappingEntity,
  UserMachineMappingEntity,
  UserShiftMappingEntity,
  ChangeRequestEntity,
  RecordRevisionEntity,
  FailureCodeEntity,
  RcaEntity,
  PendingExecutionEntity,
  ShiftHandoverEntity,
  MachineFailureCodeMappingEntity,
  WarrantyAlertEntity,
];
